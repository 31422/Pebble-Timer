...
TextLayer *new_count_down;
char new_count_down_text[10];
TextLayer *time_text;
...
void unit_marker_update_callback(Layer *me, GContext* ctx) {
  ...
  int width = 20;
  int start = 6 + (width + 7) * setting_unit;
...
void handle_init(void) {
  ...
  count_down = text_layer_create(GRect(0, 60, 144, 40));
  text_layer_set_font(count_down, fonts_get_system_font(FONT_KEY_GOTHIC_28_BOLD));
...
void handle_deinit() {
  ...
  text_layer_destroy(new_count_down);
